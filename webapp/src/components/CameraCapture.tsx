...
export async function startCamera(
  ...
  setShowCaptureButton: React.Dispatch<React.SetStateAction<boolean>>
) {
  try {
    if (videoRef.current) {
...
function CameraCapture({
  ...
  useEffect(() => {
    if (!showCaptureButton) {
      startCamera(videoRef, streamRef, setShowCaptureButton);
    }
  }, [!showCaptureButton]);
  ...
  return (
    <div>
      <video
        ref={videoRef}
        autoPlay
        style={{ display: !showCaptureButton ? "none" : "block" }}
      />
      <div
        style={{
          display: "flex",
